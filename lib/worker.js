...
module.exports = (reporter, definition) => {
  ...
  reporter.registerHelpersListeners.add('xlsx-report', async (req) => {
    return helpersScript
  })

  reporter.initializeListeners.add('xlsx-report', async () => {
  ...
  reporter.beforeRenderListeners.insert({ before: 'templates' }, definition.name, (req) => {
    if (req.template.recipe === 'xlsx-report' && !req.template.name && !req.template.shortid && !req.template.content) {
      // templates extension otherwise complains that the template is empty
      // but that is fine for this recipe
      req.template.content = 'xlsx report placeholder'
    }
  })
