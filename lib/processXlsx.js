module.exports = (reporter) => async (context, req) => {
  try {
      ...
      .reduce((prev, worksheetFile) => {
        worksheetFile.worksheet.sheetData[0]?.row?.forEach(elementRow => {
          ...
          elementRow.c?.forEach(elementCell => {
            ...
            else if (elementCell.$?.t === 's' && elementCell.v?.[0] && stringList[elementCell.v[0]].t?.[0]) {
              lastCell = false;
    ...
    stringIdxToClear.splice(0, stringIdxToClear.length);
    ...
    const newContentString = newContent.toString().replace(/(\r\n|\n|\r|\t)/gm,"").replace(/\\/g,"\\\\");
    const newContentFiles = newContentString.split('###xlsxFile###');

    const renderedStringList = JSON.parse(newContentFiles[0]);
    let val = { value: '' };
    renderedStringList.sst?.si?.forEach((str, idx) => {
      if (checkAndConvert(str.t?.[0], val)) {
        str.t[0] = val.value;
        stringIdxToClear.push(String(idx));
      }
    });
    if (stringIdxToClear.length) {
      Object.values(worksheetFiles).forEach(worksheet => {
        (worksheet.sheetData?.[0]?.row ?? []).forEach(row => {
          row.c?.filter(cell => cell.$.t === 's' && stringIdxToClear.includes(cell.v[0])).forEach(cell => {
            cell.v = [ renderedStringList.sst.si[cell.v].t[0] ];
            delete cell.$.t;
          })
        })
      })
    ...
    stringIdxToClear.forEach(i => stringList[i].t[0] = '');
    context.$xlsxReport[stringsPath] = renderedStringList;
...
function insertRow (worksheet, sheetId, calcList, row, isFirst) {
  const sheetData = worksheet.sheetData[0];
  ...
  row.c?.forEach(cell => {
    if (cell.$.t && cell.is?.t) {
      if (checkAndConvert(cell.is.t, val)) {
        cell.v = [ val.value ];
        delete cell.$.t;
        delete cell.is.t;
        delete cell.is;
  ...
  rowList.forEach((row, idx) => {
    if (idx > rowInsertIndex) {
      ...
      row.c?.forEach(cell => {
        if (cell.$?.r) {
          cell.$.r = cell.$.r.replace(/\d/g, '') + newIndex;
        }
      })
    }
  });
...
function regionFromString (value) {
  return value.split(':').map(item => ({
    col: item.replace(/\d/g, ''),
    row: parseInt(item.replace(/\D/g, '')),
  }));
}
...
function checkAndConvert (value, out) {
  out.value = value;
  /** TODO check and convert string value - { v: '', t: 's' } */
  if (value && false) {
  ...
  let v = value !== '' ? value : NaN;
  if (isNaN(v)) {
    const noSpace = String(value ?? '').replace(/\s/g, '');
    if (noSpace.split('T')[0].match(/^\d{4}[\/\-](0?[1-9]|1[012])[\/\-](0?[1-9]|[12][0-9]|3[01])$/)) {
      v = Date.parse(noSpace);
      if (!isNaN(v)) {
        const days = Math.round((v - new Date(1899, 11, 30)) / 8.64e7);
        out.value = String(parseInt((days).toFixed(10)));
      }
    }
  }
  return !isNaN(v);
